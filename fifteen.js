...
window.onload = function ()
{
	...
	{
		div[i].style.backgroundImage="url('weeknd.jpe')";
	...
	shufflebutton.onclick = function()
	{
		...
		for (var i=0; i<250; i++)
		{
			...
			if (rand == 0)
			{
				var tmp = calcUp(newBlank, oldBlank);
				if ( tmp != -1)
				{
					swap(tmp);
				}
			...
			if (rand == 1)
			{
				var tmp = calcDown(newBlank, oldBlank);
				if ( tmp != -1) 
				{
					swap(tmp);
				}
			...
			if (rand == 2)
			{
				var tmp = calcLeft(newBlank, oldBlank);
				if ( tmp != -1)
				{
					swap(tmp);
				}
			...
			if (rand == 3)
			{
				var tmp = calcRight(newBlank, oldBlank);
				if (tmp != -1)
				{
					swap(tmp);
				}
...
{
	if (calcLeft(newBlank, oldBlank) == (pos-1))
	{
		return true;
	}

	if (calcDown(newBlank, oldBlank) == (pos-1))
	{
		return true;
	}

	if (calcUp(newBlank, oldBlank) == (pos-1))
	{
		return true;
	}

	if (calcRight(newBlank, oldBlank) == (pos-1))
	{
		return true;
	}
}
function Blink()
{
	...
	if (blink == 0)
	{
		...
		body[0].style.backgroundColor = "#FF0066";
		alert('you win');
		return;
	}
	if (blink % 2)
	{
		var body = document.getElementsByTagName('body');
		body[0].style.backgroundColor = "#00FFCC";	
	}
	else
	{
		var body = document.getElementsByTagName('body');
		body[0].style.backgroundColor = "#EEEE00";
	}
...
{
	var body = document.getElementsByTagName('body');
	body[0].style.backgroundColor = "#993399";
	blink = 10;
	timer = setTimeout(Blink, 100);
}
...
function swap (pos) {
	...
	div[pos].style.top = oldBlank;
	oldBlank = temp;

	temp = div[pos].style.left;
	div[pos].style.left = newBlank;
	newBlank = temp;
}
